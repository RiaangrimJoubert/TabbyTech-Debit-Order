...
export async function fetchZohoClients({ page = 1, perPage = 50, signal } = {}) {
  // Your deployed working route is under /crm_api.
  // We still keep /api as fallback for other environments.
  const candidates = [
  ...
  throw lastErr || new Error("Failed to fetch clients from available endpoints.");
}
